...
import { AlertCircle, ArrowLeft, Download, Loader2, Palette, Save, Trash2, Plus, Check } from 'lucide-react';
...
import { Input, TitleInput } from './Inputs';
...
export const EditResume = () => {
  ...
  // FIXED UPLOAD RESUME IMAGES FUNCTION
 const uploadResumeImages = async () => {
  try {
    setIsLoading(true);
    console.log("🔄 Starting save process...");

    let thumbnailLink = ""; // Initialize as empty string
    ...
    try {
      ...
      if (thumbnailElement && document.body.contains(thumbnailElement)) {
        console.log("🎨 Creating fixed thumbnail...");
        ...
        if (fixedThumbnail) {
          ...
          // Safe cleanup
          if (fixedThumbnail.parentNode === document.body) {
          ...
          // SAFELY get thumbnailLink - check if response exists
        ...
        console.log("⚠️ Thumbnail element not available, skipping thumbnail generation");
      }
    } catch (thumbnailError) {
      console.warn("⚠️ Thumbnail generation failed, but resume will still be saved:", thumbnailError);
      // Continue without thumbnail - thumbnailLink remains empty string
    }

    // Save resume data with or without thumbnail
    console.log("💾 Saving resume data...");
    await updateResumeDetails(thumbnailLink || "");

    toast.success("Resume Saved Successfully!");
    console.log("✅ Resume saved, navigating to dashboard...");
    ...
    if (error.response) {
      ...
      toast.error(`Server error: ${error.response.data.message || 'Failed to save resume'}`);
...
const updateResumeDetails = async (thumbnailLink) => {
  try {
    ...
    // Make sure thumbnailLink is always a string
    const safeThumbnailLink = thumbnailLink || "";
    
    const updateData = {
      ...resumeData,
      thumbnailLink: safeThumbnailLink,
      completion: completionPercentage,
      updatedAt: new Date().toISOString()
    };

    console.log("📤 Sending update data with thumbnail:", safeThumbnailLink ? "Yes" : "No");
...
// In your EditResume.jsx - FIX THIS FUNCTION
  ...
  setResumeData(prev => {
    const updatedData = {
      ...
      template: {
        ...prev.template,
        theme: theme,
        // Keep existing colorPalette or set default
        colorPalette: prev.template?.colorPalette || []
      }
    };
    
    console.log("📊 Updated resume data:", updatedData);
    return updatedData;
  });
};
            ...
            <div className={containerStyles.previewContainer}>
              ...
              <div className='preview-container relative' ref={previewContainerRef}>
                <div className={containerStyles.previewInner}>
                  {/* Make sure thumbnailRef is properly attached to the preview */}
...
>
  <div className="w-full">
    <ThemeSelector 
      ...
      resumeData={resumeData}
      onClose={() => setOpenThemeSelector(false)}
      onBackToResume={() => setOpenThemeSelector(false)} // This will close theme selector and go back to resume
    />
  </div>
</Modal>
      <Modal 
      ...
      >
        <div className= "rejective">
          <div className= "text-center mb-4">
            <div className= {statusStyles.modalBadge}>
              <div className= {iconStyles.pulseDot}></div>
              <span> completion : {completionPercentage}% </span>           </div>
          </div>
        </div>
